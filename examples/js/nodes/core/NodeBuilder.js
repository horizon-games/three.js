...
(function (global, factory) {
	...
	var elements = NodeUtils_js.NodeUtils.elements,
	...
	NodeBuilder.prototype = {
		...
		createUniform: function ( shader, type, node, ns, needsUpdate, label ) {
			...
			var uniform = new NodeUniform_js.NodeUniform( {
				type: type,
		...
		include: function ( node, parent, source ) {
			...
			node = typeof node === 'string' ? NodeLib_js.NodeLib.get( node ) : node;
			...
			if ( node instanceof FunctionNode_js.FunctionNode ) {

				includesStruct = this.includes.functions;

			} else if ( node instanceof ConstNode_js.ConstNode ) {

				includesStruct = this.includes.consts;

			} else if ( node instanceof StructNode_js.StructNode ) {
			...
			if ( node ) {
				...
				if ( node instanceof FunctionNode_js.FunctionNode && parent && includes[ parent.name ] && includes[ parent.name ].deps.indexOf( node ) == - 1 ) {
					...
					if ( nodeCandidate.isNode ) {
						...
						switch ( nodeCandidate.mapping ) {
							...
							case THREE.CubeRefractionMapping:
								...
								return new CubeTextureNode_js.CubeTextureNode( nodeCandidate );
							...
							case THREE.CubeUVRefractionMapping:
								...
								return new TextureCubeNode_js.TextureCubeNode( new TextureNode_js.TextureNode( nodeCandidate ) );

								break;

							default:

								return new TextureNode_js.TextureNode( nodeCandidate );

						}

					} else if ( nodeCandidate.isVector2 ) {

						return new Vector2Node_js.Vector2Node( nodeCandidate );

					} else if ( nodeCandidate.isVector3 ) {

						return new Vector3Node_js.Vector3Node( nodeCandidate );

					} else if ( nodeCandidate.isVector4 ) {

						return new Vector4Node_js.Vector4Node( nodeCandidate );
